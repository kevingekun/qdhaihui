...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    public function checkAuth($appid = '', $appsecret = '', $token = '')
    {
        ...
        file_put_contents('./msgqueue_test.log', "appid:".json_encode($appid) . "\n\n", FILE_APPEND);
        file_put_contents('./msgqueue_test.log', "appsecret:".json_encode($appsecret) . "\n\n", FILE_APPEND);

        $result = $this->http_get(self::API_URL_PREFIX . self::AUTH_URL . 'appid=' . $appid . '&secret=' . $appsecret);

        file_put_contents('./msgqueue_test.log', "access_token_result:".json_encode($result) . "\n\n", FILE_APPEND);
    ...
    public function addTemplateMessage($tpl_id)
    {
        ...
        file_put_contents('./msgqueue_test.log', "access_token:".json_encode($this->access_token) . "\n\n", FILE_APPEND);
        file_put_contents('./msgqueue_test.log', "checkAuth:".json_encode($this->checkAuth()) . "\n\n", FILE_APPEND);

        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX . self::TEMPLATE_ADD_TPL_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        file_put_contents('./msgqueue_test.log', "result:".json_encode($result) . "\n\n", FILE_APPEND);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];

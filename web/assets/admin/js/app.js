...
var App = function () {
	...
	/****开始---CRM网站布局样式选择******/
	//布局方式
	$("#buju").change(function(){
		var buju = $(this).val();
		$.post("/Admin/Customer/buju",{buju:buju},function(msg){});
	});
	
	//头部固定方式
	$("#toubu").change(function(){
		var toubu = $(this).val();
		$.post("/Admin/Customer/toubu",{toubu:toubu},function(msg){});
	});
	
	//侧边固定方式
	$("#cebian").change(function(){
		var cebian = $(this).val();
		$.post("/Admin/Customer/cebian",{cebian:cebian},function(msg){});
	});
	
	//侧边栏位置
	$("#cweizhi").change(function(){
		var cweizhi = $(this).val();
		$.post("/Admin/Customer/cweizhi",{cweizhi:cweizhi},function(msg){});
	});
	
	//底部固定方式
	$("#dibu").change(function(){
		var dibu = $(this).val();
		$.post("/Admin/Customer/dibu",{dibu:dibu},function(msg){});
	});
	/****结束---CRM网站布局样式选择******/
...
/*******************************************handle theme colors 更换主题颜色****************************************************************************/
		//获取data_theme里面的 主题颜色 存入数据库
		$(".data_theme li").click(function(){
			var theme = $(this).attr("id");
			$.post("/Admin/Customer/theme",{theme:theme},function(msg){
				window.location.reload();
			});
		});
		
		$.post("/Admin/Customer/zdy_theme",function(color){
			$('#style_color').attr("href", "/App/Modules/Admin/Tpl/Public/assets/css/themes/" + color + ".css");
		});

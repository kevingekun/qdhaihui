$(function(){
    ...
    //模块类型16 公告
    HYD.DIY.Unit.event_type16=function(ctrldom, data){
        var $conitem = data.dom_conitem, //手机内容
            $ctrl = ctrldom, //控制内容
            tpl_con=$("#tpl_diy_con_type16").html(),//手机内容模板
            tpl_ctrl=$("#tpl_diy_ctrl_type16").html();//控制内容模板
        ...
        var reRender=function(callback){
            ...
            HYD.DIY.Unit.event_type16($ctrl,data);
            if(callback) callback();
        }
        ...
        $ctrl.find("input[name='notice']").change(function() {
            ...
            data.content.strLength = val.length;
            data.content.showtitle = val;
            reRender();
        });
    };

    //模块类型17 分类
    HYD.DIY.Unit.event_type17=function(ctrldom, data){
        var $conitem = data.dom_conitem, //手机内容
            $ctrl = ctrldom, //控制内容
            tpl_con=$("#tpl_diy_con_type17").html(),//手机内容模板
            tpl_ctrl=$("#tpl_diy_ctrl_type17").html();//控制内容模板
        ...
        var reRender=function(callback){
            ...
            HYD.DIY.Unit.event_type17($ctrl,data);
            if(callback) callback();
        }
        ...
        //分类名称修改
        $ctrl.find("input[name='classify']").change(function(){
            var index=$(this).parents("li.ctrl-item-list-li").index(),
                val=$(this).val();
             data.content.dataset[index].showtitle=val;
             reRender();
        ...
        $ctrl.find('.ctrl-item-list-li').each(function(index,ell) {
            var me = $(this);
            me.find('.colorPicker1').each(function(indexs,el) {
                var _this = $(this);
                if(indexs == 0){
                    var name=$(this).data("name"),
                        color=$(this).data("color"),
        ...
        $ctrl.find("input[name='layout']").change(function() {
            var val = $(this).val();
            $conitem.find(".members_classify").attr('class', 'members_classify layoutstyle'+val);

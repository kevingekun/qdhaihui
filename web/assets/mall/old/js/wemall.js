...
//	});
	...
	$('#user').on('click' , function (){
		...
		$.ajax({
			type : 'POST',
			url : appurl+'/Mall/Index/getOrders',
			data : {
				uid : uid
			},
			success : function (response , status , xhr){
				if(response){
					...
					$.each(json, function (index, value) {
						...
						html += '<tr><td>'+value.order_number+'</td><td class="cc">'+value.total_price+'元</td><td class="cc"><em class="'+pay_status+'">'+pay+'</em></td><td class="cc"><em class="'+order_status+'">'+order+'</em></td></tr>';
					});
...
function doProduct (id , name , price) {
	...
	if (bgcolor == '#FFFFFF') {
		...
		var html = '<li class="ccbg2" id="'+wemallId+'"><div class="orderdish"><span name="id" style="display:none">'+id+'</span><span name="title">'+name+'</span><span class="price" id="v_0">'+price+'</span><span class="price">元</span></div><div class="orderchange"><a href=javascript:addProductN("'+wemallId+'_'+price+'") class="increase"><b class="ico_increase">加一份</b></a><span class="count" id="num_1_499">1</span><a href=javascript:reduceProductN("'+wemallId+'_'+price+'") class="reduce"><b class="ico_reduce">减一份</b></a></div></li>';
		$('#ullist').append(html);
	}else{
...
function submitOrder () {
	...
	$('#ullist li').each(function () {
		...
		json += '{"id":"'+id+'","name":"'+name+'","num":"'+num+'","price":"'+price+'"},';
	...
	$.ajax({
		type : 'POST',
		url : appurl+'/Mall/Index/addOrder',
		data : {
			uid : uid,
			cartData : json,
...
function showDetail(id){
	$.ajax({
		type : 'post',
		url : appurl+'/Mall/Index/fetchGoodDetail',
		data : {
			id : id,
		},
		success : function(response , status , xhr){
			$('#mcover').show();
			var json = eval(response);
			$('#detailpic').attr('src',rooturl+'/uploads/'+json.image);
